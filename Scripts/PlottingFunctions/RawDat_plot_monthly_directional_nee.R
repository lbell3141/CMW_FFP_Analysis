...
plot_monthly_directional_nee <- function(dir_dat_avg) {
  ...
  create_monthly_nee_plot <- function(month_abbr, month_full) {
    variable_col <- paste0(month_abbr, "_nee")
  ...
  NEE_plot <- final_plot + plot_annotation(title = "Average Directional NEE (µmol CO₂ m⁻² s⁻¹)")
  return(NEE_plot)
}
