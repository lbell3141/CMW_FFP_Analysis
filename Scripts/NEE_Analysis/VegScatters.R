...
# --- Step 1: Average nee by month and 20-degree wind direction bins ---
dir_dat_avg <- dat_voi %>%
  mutate(
  ...
  summarise(nee = mean(nee, na.rm = TRUE), .groups = "drop") %>%
...
lm_ndvi <- lm(nee ~ NDVI, data = combd_df_zscores)
r2_ndvi <- summary(lm_ndvi)$r.squared

lm_chm <- lm(nee ~ CHM, data = combd_df_zscores)
r2_chm <- summary(lm_chm)$r.squared

lm_cover <- lm(nee ~ Cover, data = combd_df_zscores)
r2_cover <- summary(lm_cover)$r.squared

# Plot 1: NEE vs NDVI
p1 <- ggplot(combd_df_zscores, aes(x = NDVI, y = nee)) +
...
# Plot 2: NEE vs Canopy Height
p2 <- ggplot(combd_df_zscores, aes(x = CHM, y = nee)) +
  geom_point(color = "black", size = 0.8) +
  geom_smooth(method = "lm", se = FALSE, color = "maroon") +
  annotate("text", x = Inf, y = Inf, label = paste0("R² = ", round(r2_chm, 2)),
           hjust = 1.1, vjust = 1.5, size = 5) +
  labs(x = "Canopy Height", y = "NEE") +
  theme_minimal() +
  theme(
    text = element_text(color = "black"),
    legend.position = "none",
...
# Plot 3: NEE vs Cover
p3 <- ggplot(combd_df_zscores, aes(x = Cover, y = nee)) +
  geom_point(color = "black", size = 0.8) +
  geom_smooth(method = "lm", se = FALSE, color = "maroon") +
  annotate("text", x = Inf, y = Inf, label = paste0("R² = ", round(r2_cover, 2)),
           hjust = 1.1, vjust = 1.5, size = 5) +
  labs(x = "Canopy Cover", y = "NEE") +
  theme_minimal() +
  theme(
    text = element_text(color = "black"),
    legend.position = "none",
...
# TWI plot
#===============================================================================
# Fit linear model for NEE vs TWI
lm_twi <- lm(TWI ~ nee, data = combd_df_zscores)
r2_twi <- summary(lm_twi)$r.squared

# Create plot with updated point size and R² annotation
twi_plot <- ggplot(combd_df_zscores, aes(x = nee, y = TWI)) +
  geom_point(color = "black", size = 0.8) +
  geom_smooth(method = "lm", se = FALSE, color = "maroon") +
