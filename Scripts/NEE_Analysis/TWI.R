...
avgneevals <- dir_dat_avg%>%
  group_by(direction)%>%
  summarize(avgnee = mean(nee, na.rm = T))
...
combd_df <- inner_join(avgneevals, avgTWIvals, by = c("direction"))

#===============================================================================
# TWI plot - avg values for each direction (no month component)
#===============================================================================
twi_plot <- ggplot(combd_df, aes(x = avgnee, y = avgTWI)) +
...
# Fit linear model
lm_twi <- lm(nee ~ TWI, data = combd_df_zscores)
r2_twi <- summary(lm_twi)$r.squared

# Create plot
twi_plot <- ggplot(combd_df_zscores, aes(x = TWI, y = nee)) +
