...
mod_mm <- function(x) {
  model <- lm(nee ~ swc + temp_atmos + rel_h + ppfd + wind_sp + precip + HH_UTC, data = x)
  
  x %>%
    mutate(
      modeled_nee = predict(model, x),
      diff_nee = modeled_nee - nee
    ) %>%
    filter(modeled_nee > 0) %>%
    group_by(dir_group) %>%
    summarize(
      mod_avg_nee = mean(modeled_nee, na.rm = TRUE),
      real_avg_nee = mean(nee, na.rm = TRUE),
      diff_avg_nee = mean(diff_nee, na.rm = TRUE),
      .groups = "drop"
    ) %>%
...
all_diff_avg_nee <- unlist(lapply(mm_mod_list, \(df) df$diff_avg_nee))
overall_min <- min(all_diff_avg_nee, na.rm = TRUE)
overall_max <- max(all_diff_avg_nee, na.rm = TRUE)

#===============================================================================
# Plotting Function
#===============================================================================

plot_nee_difference <- function(df, month) {
  ggplot(df) +
    geom_col(
      aes(x = direction, y = 5, fill = diff_avg_nee),
      width = 20
    ) +
    coord_polar() +
...
plot_list <- Map(plot_nee_difference, mm_mod_list, month_names)
...
# Display
final_plot +
  plot_annotation(title = "Directional Residual NEE") +
